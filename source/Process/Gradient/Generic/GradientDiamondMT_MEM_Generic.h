...
*   ULIS
*__________________
* @file         GradientDiamondMT_MEM_Generic.h
...
template< typename T >
void
InvokeGradientDiamondMT_MEM_Generic(
...
)
{
    ...
    FVec2F dir = delta / length;
    FVec2F orth( -dir.y, dir.x );
    length = ( p0 - ( ( p0 + dir * length ) * 0.5f + ( p0 + orth * length ) * 0.5f ) ).Distance();
    const float angle = atan2f( dir.y, dir.x );
    const float cos = cosf( angle + FMath::kPIf / 4 );
    const float sin = sinf( angle + FMath::kPIf / 4 );
    dir.x = cos;
    dir.y = sin;
    dir.Normalize();
    orth = FVec2F( -dir.y, dir.x );
    ...
    for( int x = x1; x < x2; ++x ) {
        ...
        // p0.x + orth.x * t = src.x + dir.x * u
        // p0.y + orth.y * t = src.y + dir.y * u
        ...
        u = FMath::Abs( ( ( p0.y - src.y ) * dir.x - ( p0.x - src.x ) * dir.y ) / ( orth.x * dir.y - orth.y * dir.x ) );
        ...
        const ufloat param0 = FMath::Clamp( ( t + dither * cargs->dithering * MaxType< T >() ) / length, 0.f, 1.f );
        const ufloat param1 = FMath::Clamp( ( u + dither * cargs->dithering * MaxType< T >() ) / length, 0.f, 1.f );
        const ufloat param = FMath::Max( param0, param1 );
...
ULIS_DEFINE_GENERIC_COMMAND_SCHEDULER_FORWARD_SIMPLE( ScheduleGradientDiamondMT_MEM_Generic, FSimpleBufferJobArgs, FGradientCommandArgs, &InvokeGradientDiamondMT_MEM_Generic< T > )
ULIS_NAMESPACE_END
