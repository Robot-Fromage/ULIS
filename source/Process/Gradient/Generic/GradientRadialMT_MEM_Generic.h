...
*   ULIS
*__________________
* @file         GradientRadialMT_MEM_Generic.h
...
template< typename T >
void
InvokeGradientRadialMT_MEM_Generic(
...
)
{
    ...
    for( int x = x1; x < x2; ++x ) {
        ...
        const ufloat param = FMath::Clamp( ( dist + dither * cargs->dithering * MaxType< T >() ) / length, 0.f, 1.f );
...
ULIS_DEFINE_GENERIC_COMMAND_SCHEDULER_FORWARD_SIMPLE( ScheduleGradientRadialMT_MEM_Generic, FSimpleBufferJobArgs, FGradientCommandArgs, &InvokeGradientRadialMT_MEM_Generic< T > )
ULIS_NAMESPACE_END
