...
InvokeTransformPerspectiveMT_NN_MEM_Generic( uint8* iDst, int32 iLine, std::shared_ptr< const FTransformCommandArgs > iInfo ) {
...
template< typename T > void
TransformPerspectiveMT_NN_MEM_Generic( std::shared_ptr< const FTransformCommandArgs > iInfo ) {
    const FTransformCommandArgs&   info        = *iInfo;
    ...
    ULIS_MACRO_INLINE_PARALLEL_FOR( info.perfIntent, info.pool, info.blocking
                                   , info.dst_roi.h
                                   , InvokeTransformPerspectiveMT_NN_MEM_Generic< T >
