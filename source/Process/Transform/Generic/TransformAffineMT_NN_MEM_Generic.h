...
InvokeTransformAffineMT_NN_MEM_Generic( uint8* iDst, int32 iLine, std::shared_ptr< const FTransformCommandArgs > iInfo ) {
...
template< typename T > void
TransformAffineMT_NN_MEM_Generic( std::shared_ptr< const FTransformCommandArgs > iInfo ) {
    const FTransformCommandArgs&   info        = *iInfo;
