...
*   ULIS
*__________________
* @file         ResizeMT_Bilinear_SSE_RGBA8.cpp
...
void
InvokeResizeMT_Bilinear_SSE_RGBA8(
...
)
{
    ...
    for( int x = 0; x < cargs->dstRect.w; ++x ) {
        ...
        const int   bot     = top + 1;
        const Vec4f tx      = point_in_src.x - left + 0.5f;
        const Vec4f ux      = 1.f - tx;
        const Vec4f ty      = point_in_src.y - top + 0.5f;
...
ULIS_DEFINE_RESIZE_COMMAND_SPECIALIZATION( ResizeMT_Bilinear_SSE_RGBA8 )
