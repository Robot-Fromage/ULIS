...
template< typename T >
void
InvokeBlendMTProcessScanline_Misc_MEM_Generic_Subpixel( const uint8* iSrc, uint8* iBdp, int32 iLine, const uint32 iSrcBps, const FBlendArgs* iArgs ) {
...
template< typename T >
void
ScheduleBlendMT_Misc_MEM_Generic_Subpixel( const FBlendArgs* iArgs ) {
    const FBlendArgs&   info        = *iInfo;
...
template< typename T >
void
InvokeBlendMTProcessScanline_Misc_MEM_Generic( const uint8* iSrc, uint8* iBdp, int32 iLine, const FBlendArgs* iArgs ) {
    const FBlendArgs&   info    = *iInfo;
    const FFormat&  fmt     = info.source->FormatInfo();
    const uint8*        src     = iSrc;
    uint8*              bdp     = iBdp;
...
template< typename T >
void
ScheduleBlendMT_Misc_MEM_Generic( const FBlendArgs* iArgs ) {
    const FBlendArgs&   info        = *iInfo;
    const uint8*        src         = info.source->Bits();
    uint8*              bdp         = info.backdrop->Bits();
    const uint32         src_bps     = info.source->BytesPerScanLine();
